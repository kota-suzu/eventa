...
- [x] docker-compose.ymlの非推奨警告を修正（`version`属性を削除）
- [x] debug.jsのuseEffect依存配列の問題を修正（useMemoで包む）
- [x] imgタグをNext.jsの`<Image>`コンポーネントに置き換え
- [ ] より良いエラーハンドリング（特にAPI通信でのエラー処理）の実装
- [ ] ページネーション機能の実装
- [ ] フィルタリング機能の強化
- [ ] ローディング状態の改善とスケルトンUIの導入
- [ ] i18n（国際化）対応の強化
- [ ] モバイルレスポンシブデザインの最適化
- [ ] チケット購入フローの実装
- [ ] ソーシャル共有機能の追加
...
- [ ] RSpec テストの修正（69個のテストが失敗中）
  - [ ] モデルバリデーションテストの修正
  - [ ] JWT認証関連テストの修正
  - [ ] コントローラーテストの応答ステータスコード修正
  - [ ] Redis連携のテスト修正

- [ ] Rails 8互換性対応
  - [x] `migration_context`問題への対応
  - [ ] JWT認証関連のバグ修正
...
- [ ] OAuth認証（Google, Facebook, Twitterなど）の導入
- [ ] 決済連携機能の実装
- [ ] プッシュ通知機能
- [ ] リアルタイム更新機能
- [ ] アナリティクス機能 