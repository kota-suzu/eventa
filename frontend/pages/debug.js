...
const DebugPage = () => {
  ...
  const sendAxiosRequest = async () => {
    ...
    try {
      const endpoint = useAbsolutePath 
        ? 'auths/register' 
        : getEndpointUrl('auths/register');
        
      console.log(`Axios APIリクエスト送信先: ${useAbsolutePath ? api.defaults.baseURL + '/' + endpoint : endpoint}`);
  ...
  const sendCustomRequest = async () => {
    ...
    try {
      ...
      console.log(`カスタムリクエスト送信先(${selectedApiClient}): ${useAbsolutePath ? endpoint : api.defaults.baseURL + '/' + endpoint}`);
    ...
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '20px', padding: '15px', backgroundColor: '#f8f9fa', borderRadius: '4px' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <tbody>
            <tr>
              <td style={{ padding: '8px', borderBottom: '1px solid #ddd' }}><strong>環境変数</strong></td>
              <td style={{ padding: '8px', borderBottom: '1px solid #ddd' }}>{apiConfig.envVars.NEXT_PUBLIC_API_URL}</td>
            </tr>
            <tr>
              <td style={{ padding: '8px', borderBottom: '1px solid #ddd' }}><strong>実行環境</strong></td>
              <td style={{ padding: '8px', borderBottom: '1px solid #ddd' }}>{apiConfig.envVars.NODE_ENV}</td>
            </tr>
            <tr>
              <td style={{ padding: '8px', borderBottom: '1px solid #ddd' }}><strong>auth.js ベースURL</strong></td>
              <td style={{ padding: '8px', borderBottom: '1px solid #ddd' }}>{apiConfig.auth.baseURL}</td>
            </tr>
            <tr>
              <td style={{ padding: '8px', borderBottom: '1px solid #ddd' }}><strong>api.js ベースURL</strong></td>
              <td style={{ padding: '8px', borderBottom: '1px solid #ddd' }}>{apiConfig.api.baseURL}</td>
            </tr>
            <tr>
              <td style={{ padding: '8px', borderBottom: '1px solid #ddd' }}><strong>Credentials</strong></td>
              <td style={{ padding: '8px', borderBottom: '1px solid #ddd' }}>
                auth.js: {apiConfig.auth.withCredentials ? 'true' : 'false'}, 
                api.js: {apiConfig.api.withCredentials ? 'true' : 'false'}
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px', flexWrap: 'wrap' }}>
          ...
          <div>
            ...
            <button 
              ...
              disabled={loading}
              style={{ marginLeft: '10px', padding: '5px 10px', background: '#17a2b8', color: 'white', border: 'none', borderRadius: '4px' }}
            >
              リクエスト送信
            </button>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <div>
            ...
            <input 
              ...
              onChange={e => setTestUserData({...testUserData, password: e.target.value, password_confirmation: e.target.value})}
              style={{ marginLeft: '10px', width: '300px', padding: '5px' }}
            />
          </div>
        </div>
      ...
      {loading && <div style={{ padding: '20px', textAlign: 'center', backgroundColor: '#f8f9fa', borderRadius: '4px' }}>読み込み中...</div>}
      
      {result && (
        <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#f8f9fa', borderRadius: '4px' }}>
          <h2>結果 ({result.type} {result.apiClient ? `- ${result.apiClient}` : ''})</h2>
          {result.error ? (
            <div style={{ color: 'red' }}>
              <h3>エラー</h3>
              <pre style={{ backgroundColor: '#ffebee', padding: '10px', borderRadius: '4px', overflowX: 'auto' }}>{result.error}</pre>
              {result.details && (
                <div>
                  <h4>詳細</h4>
                  <pre style={{ backgroundColor: '#f5f5f5', padding: '10px', borderRadius: '4px', overflowX: 'auto' }}>{JSON.stringify(result.details, null, 2)}</pre>
                </div>
              )}
              {result.requestConfig && (
                <div>
                  <h4>リクエスト設定</h4>
                  <pre style={{ backgroundColor: '#f5f5f5', padding: '10px', borderRadius: '4px', overflowX: 'auto' }}>{JSON.stringify(result.requestConfig, null, 2)}</pre>
                </div>
              )}
            </div>
          ) : (
            <div>
              {result.status && <div><strong>ステータス:</strong> {result.status}</div>}
              {result.ok !== undefined && <div><strong>成功:</strong> {result.ok ? 'はい' : 'いいえ'}</div>}
              {result.requestUrl && <div><strong>リクエストURL:</strong> {result.requestUrl}</div>}
              <pre style={{ backgroundColor: '#e8f5e9', padding: '10px', borderRadius: '4px', marginTop: '10px', overflowX: 'auto' }}>{JSON.stringify(result.data, null, 2)}</pre>
            </div>
          )}
        </div>
      )}
      
      <div style={{ marginTop: '40px', backgroundColor: '#e3f2fd', padding: '15px', borderRadius: '4px' }}>
        <h3>デバッグのヒント</h3>
        <ul>
          <li><strong>ネットワークエラー</strong>: APIサーバーへの接続ができていない可能性があります。CORSの設定やネットワーク接続を確認してください。</li>
          <li><strong>CORS問題</strong>: API側で適切なCORS設定（origins, credentials）が必要です。</li>
          <li><strong>パス問題</strong>: ベースURLとエンドポイントの結合で重複や不足がないか確認してください。</li>
          <li><strong>Dockerコンテナ間接続</strong>: コンテナ内からの接続とブラウザからの接続はURLが異なる場合があります。</li>
          <li><strong>環境変数</strong>: NEXT_PUBLIC_API_URLが正しく設定されていることを確認してください。</li>
          <li><strong>APIクライアント重複</strong>: auth.jsとapi.jsの両方にAPIクライアントが定義されています。どちらを使うかを切り替えて試してください。</li>
        </ul>
      </div>
    </div>
  );
